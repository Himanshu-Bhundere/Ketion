...
class KetionSlashCommand extends SuperEditorSlashCommandOption {
  final List<EditRequest> Function(String nodeId, Document document)?
      getRequestsWithDoc;
  ...
  KetionSlashCommand({
    ...
    this.getRequestsWithDoc,
          ...
          getEditRequests: (nodeId) =>
              [], // We will override this behavior in the controller
        );
...
class KetionSlashCommandRegistry {
  static Map<String, dynamic> _getInheritedMetadata(String nodeId, Document doc) {
    final node = doc.getNodeById(nodeId);
    if (node != null && node.metadata.containsKey('parentBlockId')) {
      return {'parentBlockId': node.metadata['parentBlockId']};
    }
    return {};
  }
  ...
  static final List<KetionSlashCommand> _commands = [
    KetionSlashCommand(
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getParagraphTypeRequests(nodeId, doc, header1Attribution),
    ),
    KetionSlashCommand(
      id: 'h2',
      title: 'Heading 2',
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getParagraphTypeRequests(nodeId, doc, header2Attribution),
    ),
    KetionSlashCommand(
      id: 'h3',
      title: 'Heading 3',
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getParagraphTypeRequests(nodeId, doc, header3Attribution),
    ),
    KetionSlashCommand(
      id: 'quote',
      title: 'Quote',
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getParagraphTypeRequests(nodeId, doc, blockquoteAttribution),
    ),
    KetionSlashCommand(
      id: 'divider',
      title: 'Divider',
      ...
      getRequestsWithDoc: (nodeId, doc) => [
        InsertNodeBeforeNodeRequest(
          ...
          newNode: HorizontalRuleNode(id: Editor.createNodeId()), // SuperEditor's HorizontalRuleNode doesn't explicitly expose metadata in its basic constructor sometimes, but wait...
        ),
      ],
    ),
    KetionSlashCommand(
      ...
      aliases: const ['code', '```'],
      getRequestsWithDoc: (nodeId, doc) =>
          _getParagraphTypeRequests(nodeId, doc, codeAttribution),
    ),
    KetionSlashCommand(
      id: 'checklist',
      title: 'Checklist',
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getListRequests(nodeId, doc, 'checklist'),
    ),
    KetionSlashCommand(
      id: 'bulleted_list',
      title: 'Bulleted List',
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getListRequests(nodeId, doc, 'bullet'),
    ),
    KetionSlashCommand(
      id: 'numbered_list',
      title: 'Numbered List',
      ...
      getRequestsWithDoc: (nodeId, doc) =>
          _getListRequests(nodeId, doc, 'numbered'),
    ),
    ...
    KetionSlashCommand(
      ...
      getRequestsWithDoc: (nodeId, doc) {
        ...
        if (node == null) return [];
        return [
          ConvertToCalloutRequest(nodeId: nodeId),
        ];
      },
    ),
    KetionSlashCommand(
      ...
      isSupported: true,
      getRequestsWithDoc: (nodeId, doc) {
        final newChildId = Editor.createNodeId();
        final metadata = _getInheritedMetadata(nodeId, doc);
        return [
          ..._getParagraphMetadataRequests(nodeId, doc, {
          ...
          InsertNodeAfterNodeRequest(
            ...
            newNode: ParagraphNode(
              id: newChildId,
              text: AttributedText(),
              metadata: {'parentBlockId': nodeId, ...metadata},
            ),
          ),
      ...
      getRequestsWithDoc: (nodeId, doc) => [
        InsertNodeAfterNodeRequest(
          ...
          newNode: ImageNode(
            ...
            imageUrl: '',
            metadata: _getInheritedMetadata(nodeId, doc),
          ),
        ),
      ],
    ),
    ...
    KetionSlashCommand(
      ...
      isSupported: true,
      getRequestsWithDoc: (nodeId, doc) => [], // Dialog handles insertion
    ),
    KetionSlashCommand(
      id: 'table',
      title: 'Table',
        ...
        return [
          InsertNodeBeforeNodeRequest(
            ...
            newNode: KetionTableNode(
              id: Editor.createNodeId(),
              metadata: {
                ..._getInheritedMetadata(nodeId, doc),
                'columnCount': 3,
                'rows': [
                  {
                    'id': const Uuid().v4(),
  ...
  static List<EditRequest> _getListRequests(
      String nodeId, Document document, String listType,) {
    ...
    if (listType == 'bullet' || listType == 'numbered') {
      final newType = listType == 'numbered'
  ...
  static List<EditRequest> _getParagraphTypeRequests(
      String nodeId, Document document, NamedAttribution blockType,) {
    final node = document.getNodeById(nodeId);
    if (node == null) return [];
  ...
  static List<EditRequest> _getParagraphMetadataRequests(String nodeId,
      Document document, Map<String, dynamic> metadataOverrides,) {
    final node = document.getNodeById(nodeId);
    if (node == null) return [];
    ...
    final currentMetadata = node is ParagraphNode
        ? Map<String, dynamic>.from(node.metadata)
        : <String, dynamic>{};
    final newMetadata = {
      ...currentMetadata,
      ...metadataOverrides,
