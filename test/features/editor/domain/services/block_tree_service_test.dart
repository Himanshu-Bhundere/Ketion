...
void main() {
  ...
  Block createBlock({required String id, String? parentId, required double position}) {
    return Block(
      id: id,
      pageId: 'page_1',
  ...
  group('BlockTreeService', () {
    ...
    test('indentBlock moves block into previous sibling', () {
      ...
      expect(updated[0].position, 0.0); // Assuming SiblingPositionManager gives 0.0 for first child
    });
    ...
    test('outdentBlock moves block after its parent', () {
      ...
      expect(updated[0].position, 5.0); // SiblingPositionManager should give middle of 0 and 10
    });
    ...
    test('moveBlock handles DropIntent.after correctly', () {
      ...
      // Move 'c' after 'a'
      final updated = BlockTreeService.moveBlock('c', const DropIntent.after('a'), blocks);
      expect(updated.length, 1);
      expect(updated[0].parentBlockId, isNull);
    ...
    test('moveBlock prevents cycles', () {
      ...
      final updated = BlockTreeService.moveBlock('parent', const DropIntent.child('child'), blocks);
