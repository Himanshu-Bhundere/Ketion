...
class EditorPersistenceCoordinator {
  ...
  final _successController =
      StreamController<EditorPersistenceMutation>.broadcast();
    ...
    if (mutation is UpdateBlockMutation) {
      ...
      if (existing != null) {
        ...
        _pendingUpdates[mutation.blockId] = UpdateBlockMutation(
          ...
          expectedVersion:
              existing.expectedVersion, // Keep original expected version
        ...
        if (lastQueued != null && lastQueued is UpdateBlockMutation) {
          ...
          _queue[lastQueuedIndex] = UpdateBlockMutation(
            ...
            position: mutation.position,
            expectedVersion:
                lastQueued.expectedVersion, // Keep original expected version
          ...
          final rebased =
              mutation.rebase(_pendingVersionState) as UpdateBlockMutation;
  ...
  void _applyTransitions(EditorPersistenceMutation mutation) {
    for (final transition in mutation.versionTransitions) {
      final currentExpected = _pendingVersionState[transition.blockId] ??
          _getPreconditionVersion(mutation, transition.blockId) ??
          0;
  ...
  int? _getPreconditionVersion(
      EditorPersistenceMutation mutation, String blockId,) {
  ...
  bool _dependsOn(
      EditorPersistenceMutation later, EditorPersistenceMutation earlier,) {
    return later.affectedBlockIds
        .intersection(earlier.affectedBlockIds)
        .isNotEmpty;
  }
    ...
    try {
      while (_queue.isNotEmpty || _pendingUpdates.isNotEmpty) {
        ...
        if (_isBlocked(mutation)) {
          debugPrint(
              'KETION: Mutation blocked due to dependencies: ${mutation.runtimeType}',);
      ...
      _isProcessing = false;
      if (_queue.isNotEmpty || _pendingUpdates.isNotEmpty) {
        _scheduleProcessing();
      }
    }
  }
  ...
  Future<bool> flush() async {
    ...
    if (previousState == CoordinatorState.active) {
