...
void main() {
  ...
  testWidgets('Table drag and drop reordering of rows and columns works and persists', (tester) async {
    final testPage = page_entity.Page(
      id: pageId,
      title: 'Test',
      createdAt: DateTime.now(),
    ...
    final initialBlocks = [
      Block(
        ...
        data: jsonEncode({
          'columnCount': 2,
          'rows': <Map<String, dynamic>>[
            {'id': 'row_1', 'cells': <Map<String, dynamic>>[{'id': '1_1', 'spans': <dynamic>[{'text': 'R1C1'}]}, {'id': '1_2', 'spans': <dynamic>[{'text': 'R1C2'}]}]},
            {'id': 'row_2', 'cells': <Map<String, dynamic>>[{'id': '2_1', 'spans': <dynamic>[{'text': 'R2C1'}]}, {'id': '2_2', 'spans': <dynamic>[{'text': 'R2C2'}]}]},
            {'id': 'row_3', 'cells': <Map<String, dynamic>>[{'id': '3_1', 'spans': <dynamic>[{'text': 'R3C1'}]}, {'id': '3_2', 'spans': <dynamic>[{'text': 'R3C2'}]}]},
          ],
        }),
        position: 1000,
        createdAt: DateTime.now(),
    ...
    // Test Reorder Row: move row 0 to position 1
    editor.execute([
      UpdateTableRequest(
        nodeId: tableNode.id,
    ...
    final tableBlockAfterReorder = testBlocks.firstWhere((b) => b.type == 'table');
    final dataAfterReorder = jsonDecode(tableBlockAfterReorder.data);
    final row0 = dataAfterReorder['rows'][0];
    expect(row0['id'], 'row_2');
    final row1 = dataAfterReorder['rows'][1];
    expect(row1['id'], 'row_1');
  });
}
