...
import 'package:ketion/features/editor/domain/models/media_nodes.dart';
...
void main() {
  ...
  testWidgets('Reminder persistence and structural round-trip', (tester) async {
    ...
    // Simulate convert to Reminder via slash command
    final List<SlashCommandOption> options = hostState.getSlashOptionsForTesting('') as List<SlashCommandOption>;
    final reminderOption = options.firstWhere((o) => o.title == 'Reminder');
    
    List<EditRequest> requests = [];
    if (reminderOption is KetionSlashCommand) {
      requests = reminderOption.getEditRequestsWithDoc(currentParaId, document);
    } else if (reminderOption is SuperEditorSlashCommandOption) {
      requests = reminderOption.getEditRequests(currentParaId);
    }
    ...
    expect(document.last is KetionReminderNode, isTrue);
    final reminderNode = document.last as KetionReminderNode;
    expect(reminderNode.title, 'New Reminder');
    expect(reminderNode.completed, false);
    expect(reminderNode.dueAt, isNotEmpty);

    // Verify SQLite block persistence (DummyBlockRepository)
    expect(testBlocks.length, 2);
    final reminderBlock = testBlocks.last;
    expect(reminderBlock.type, 'reminder'); 
    
    final data = jsonDecode(reminderBlock.data);
    expect(data['title'], 'New Reminder'); 
    expect(data['completed'], false);
    expect(data['dueAt'], isNotEmpty);
    ...
    expect(documentReopened.last is KetionReminderNode, isTrue);
    final reopenedReminder = documentReopened.last as KetionReminderNode;
    expect(reopenedReminder.title, 'New Reminder');
    expect(reopenedReminder.completed, false);
    expect(reopenedReminder.dueAt, isNotEmpty);
  });
}
