...
void main() {
  ...
  testWidgets('Media persistence and structural round-trip', (tester) async {
    ...
    // Provide blocks representing media
    final initialBlocks = [
      Block(
        id: 'video1',
        pageId: pageId,
    ...
    final document = hostState.document as Document;

    expect(document.nodeCount, 4);

    // In a Document, nodes is an iterable, and we can access them by index using elementAt or converting to list.
    final nodesList = document.toList();

    // Verify Video
    final videoNode = nodesList[0] as KetionVideoNode;
    expect(videoNode.attachmentId, 'att-video-1');

    // Verify Audio
    final audioNode = nodesList[1] as KetionAudioNode;
    expect(audioNode.attachmentId, 'att-audio-1');

    // Verify PDF
    final pdfNode = nodesList[2] as KetionPdfNode;
    expect(pdfNode.attachmentId, 'att-pdf-1');

    // Verify File
    final fileNode = nodesList[3] as KetionFileNode;
    expect(fileNode.attachmentId, 'att-file-1');
  });
}
