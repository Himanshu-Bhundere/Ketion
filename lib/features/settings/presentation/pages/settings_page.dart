      ...
      body: ref.watch(appSettingsProvider).when(
        data: (settings) {
          ...
          ListTile(
            ...
            subtitle: const Text('Export database and attachments locally'),
            onTap: () async {
              try {
                ScaffoldMessenger.of(context).showSnackBar(
          ...
          ListTile(
            ...
            subtitle: const Text('Restore database and attachments from a local file'),
            onTap: () async {
              try {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Select backup file...')),
                );
                ...
                if (context.mounted) {
                  await showDialog<void>(
                    ...
                    builder: (context) => AlertDialog(
                      ...
                      content: const Text('The backup has been restored. Please restart the app completely to load the restored database.'),
                      actions: [
                        TextButton(
                          onPressed: () => context.pop(),
                          child: const Text('OK'),
          ...
          ListTile(
            ...
            trailing: DropdownButton<String>(
              ...
              onChanged: (String? newValue) {
                if (newValue != null) {
                  ref.read(appSettingsProvider.notifier).updateSyncFrequency(newValue);
                }
              },
              items: <String>['Manual', '15 minutes', '30 minutes', '1 hour', 'Daily']
                  .map<DropdownMenuItem<String>>((String value) {
                return DropdownMenuItem<String>(
                  value: value,
                  child: Text(value),
