...
class RichTextController extends TextEditingController {
  ...
  TextSpan buildTextSpan({
    ...
    final sortedBoundaries = boundaries.where((b) => b <= text.length).toList()
      ..sort();
    ...
    for (int i = 0; i < sortedBoundaries.length - 1; i++) {
      ...
      final activeSpans = _spans
          .where((s) => s.offset <= start && (s.offset + s.length) >= end)
          .toList();
    ...
    switch (span.type) {
      ...
      case 'color':
        if (span.value != null) {
          try {
            final color =
                Color(int.parse(span.value!.replaceFirst('#', '0xFF')));
